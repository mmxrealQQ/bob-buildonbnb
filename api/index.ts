...
const AGENT_CARD = {
  ...
  url: "https://bob-buildonbnb.vercel.app",
  provider: { organization: "BOB", url: "https://bob-buildonbnb.vercel.app" },
  version: "1.0.0",
...
function agentRegistration() {
  const reg = {
    ...
    token: BOB_TOKEN,
    services: [
      { name: "A2A", version: "0.3.0", endpoint: "https://bob-buildonbnb.vercel.app" },
      { name: "agentWallet", endpoint: `eip155:56:${WALLET}` },
      { name: "Web", endpoint: "https://bob-buildonbnb.vercel.app" },
    ],
...
function handleA2A(body: any) {
  ...
  if (method === "message/send") {
    return jsonRpc(id, {
      status: "completed",
      artifacts: [{
        parts: [{ type: "text", text: "Hey! I'm BOB Build On BNB. Check out the dashboard at https://bob-buildonbnb.vercel.app to explore 80+ BNB Chain AI tools." }],
      }],
    });
  }
...
export default async function handler(req: Request) {
  const url = new URL(req.url, "https://bob-buildonbnb.vercel.app");
